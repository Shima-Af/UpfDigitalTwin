...
\begin{equation}
  ...
  \alpha = 0.12~\text{Gbps per normalised unit}.
  \label{eq:dt_alpha}
\end{equation}
...
\begin{description}
  \item[\code{is\_safe} (QoS preserved).] True iff the predicted downlink delay
    ...
    \begin{equation}
      ...
      \label{eq:dt_is_safe}
    \end{equation}
    with the default budget $D_{\max} = 200~\mu$s and zero-tolerance
    $L_{\max} = 0$ packets per interval. \dpdk{} is safe across the entire
    measured load range; \usr{} is safe only below its knee.
...
(default 0.05\,W) while it is not the serving \upf{}.
...
$\lambda_{\mathrm{be}} = 91$\,Mbps and $\lambda_{\mathrm{qos}} = 81$\,Mbps. The
...
\end{equation}
giving $\lambda_{\mathrm{dec}} = 71$\,Mbps here (QoS-limited).
...
\begin{equation}
  ...
  \label{eq:dt_band}
\end{equation}
With the forecaster's mean absolute error of $\approx 13.3$\,Mbps for the
operating cluster count, the band is $\approx 26.6$\,Mbps, giving an
upper switch point $\lambda_{\uparrow} = 71$\,Mbps and a lower one
$\lambda_{\downarrow} = 44$\,Mbps. The controller switches \usr{}$\to$\dpdk{}
only when the predicted load rises above $\lambda_{\uparrow}$, and back only
when it falls below $\lambda_{\downarrow}$.
...
\begin{figure}[htbp]
  ...
  \caption{Operating thresholds derived from the surrogate. \usr{} power
    ...
    ($91$\,Mbps); the QoS limit ($81$\,Mbps) is where \usr{} first becomes
    unsafe. The decision threshold $\lambda_{\uparrow}=71$\,Mbps is the
    binding (QoS) limit minus a 10\,Mbps margin; the shaded band down to
    $\lambda_{\downarrow}=44$\,Mbps is the forecast-derived hysteresis region.
...
\subsection{Why energy-aware switching pays}
...
right-skewed: roughly 78\,\% of all cluster-steps fall below the
decision threshold, i.e.\ in the regime where \usr{} is both safe and cheaper
than \dpdk{}. A controller that exploits this---running \usr{} at low load and
\dpdk{} only when load rises---stands to recover most of \dpdk{}'s fixed power
cost without violating QoS.
...
\begin{figure}[htbp]
  ...
  \caption{Distribution of calibrated offered load across all clusters and
    time windows. About 78\,\% of cluster-steps lie below the derived decision
    threshold (dashed), i.e.\ in the regime where \usr{} is the safe and
    cheaper action.}
...
power trace (lower panel) accordingly sits near \dpdk{}'s 0.82\,W floor during
high-load excursions and drops into \usr{}'s low-power regime otherwise. The
band visibly prevents single-sample flips around the threshold.
...
three controllers over all $10{,}090$ steps. Static \usr{} saves
37\,\% of energy versus static \dpdk{} but is \emph{unsafe on
18.8\,\% of its steps}---the direct consequence of ignoring the saturation
knee. The hysteresis rule, by respecting the surrogate-derived QoS limit,
saves \emph{more} energy (49\,\%) while cutting the unsafe rate to
1.7\,\%, by ceding to \dpdk{} exactly on the minority of high-load steps where
\usr{} would fail. This is the central demonstration that the twin behaves as
intended: it exposes the energy--safety trade-off quantitatively, and a
controller that consumes its safety signal dominates one that does not.

\begin{table}[htbp]
  \centering
  \caption{Aggregate metrics over $10{,}090$ cluster-steps (non-learning
    controllers; learned controllers and oracle bound are deferred to
    Chapter~\ref{chap:controllers}). Energy includes switching spikes.}
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    Static \dpdk{} & $2067.1$ & $0.819$ & $0.0$  & $0.0$  & $0.0$  \\
    Static \usr{}  & $1296.1$ & $0.464$ & $37.3$ & $18.8$ & $100.0$ \\
    Hysteresis     & $1049.9$ & $0.378$ & $49.2$ & $1.7$  & $75.6$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{Aggregate comparison of the non-learning controllers.
    ...
    (b)~unsafe \usr{} rate; (c)~\usr{} usage. Hysteresis saves the most energy
    \emph{and} is nearly always safe, whereas static \usr{} is cheap but
    frequently unsafe.}
...
\begin{enumerate}
  ...
  \item \textbf{Demonstration.} Driven by a transparent hysteresis controller,
    the twin recovers 49\,\% of \dpdk{}'s energy while remaining safe on
    98.3\,\% of \usr{} steps, quantitatively exposing the energy--safety
    trade-off that learned controllers will later be trained to navigate.
\end{enumerate}
